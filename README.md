...
대분류·중분류 및 Neo4j 적재 노트북은 홍기표/input/recipes_10000_cleaned.jsonl을 입력으로 사용하며, 해당 파일에서 실제로 읽은 레시피 수는 9,997건입니다.
...
노트북은 9,997건에서 9,381건으로 줄어든 전체 결과만 출력하며, 616건의 제거 사유별 세부 건수는 별도로 계산하지 않습니다.
...
임베딩은 홍기표/백터임베딩.ipynb에서 별도로 수행합니다.
...
| 임베딩 문서 수 | 9,381 |
| 배치 크기 | 100 |
| 백업 파일 | 홍기표/recipe_vectors_backup.pkl |
...
홍기표/agent.py는 사용자의 질문을 동일한 임베딩 모델로 벡터화한 뒤 db.index.vector.queryNodes를 호출하여 의미적으로 가까운 레시피를 검색합니다. 검색 결과는 레시피 정보와 재료를 문맥으로 구성하여 답변 생성에 사용합니다.

> 참고: 임베딩 백업 산출물은 확인되지만, 임베딩 노트북의 저장된 실행 로그에는 OpenAI 사용량 제한 오류와 Neo4j 인증 오류가 남아 있습니다. 배포 환경에서 벡터 검색을 사용할 때는 임베딩 적재와 dish_embedding_index 상태를 다시 확인해야 합니다.
...
현재 Streamlit 본체의 검색은 recipe_graph.py에 정의된 Cypher 조회를 사용합니다. 벡터 기반 자연어 추천은 별도 홍기표/agent.py 모듈에서 구현되어 있으며, 앱 본체에 직접 연결된 구성과는 구분됩니다.
